...
public extension Theme {
    ...
    static var localTheme: Self {
        Theme(
            htmlFactory: LocalHTMLFactory(),
            resourcePaths: []
        )
    }
}

private struct LocalHTMLFactory<Site: Website>: HTMLFactory {
...
private extension Node where Context == HTML.BodyContext {
    ...
    static func itemList<T: Website>(for items: [Item<T>], on site: T) -> Node {
        return .ul(
            .class("ul-writing-container"),
            .forEach(items) { item in
                .li(
                    .class("li-writing-item"),
                    .article(
                        .h4(.a(
                            .href(item.path),
                            .text(item.title)
                            )),
                        .h6(.text(item.date.formattedDate())),
